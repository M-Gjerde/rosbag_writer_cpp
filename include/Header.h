...
std::vector<uint8_t> serialize_time(uint64_t val) {
    uint32_t sec = val / 1000000000;
    uint32_t nsec = val % 1000000000;
    std::vector<uint8_t> bytes = serialize_uint32(sec);
    std::vector<uint8_t> nsec_bytes = serialize_uint32(nsec);
...
public:
    ...
    void set_time(const std::string& name, uint64_t value) {
        data[name] = serialize_time(value);
    }

    int write(std::ostream& dst, RecordType opcode = RecordType::MSGDATA) {
